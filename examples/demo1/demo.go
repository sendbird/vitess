...
func runCluster() {
	cluster = &vttest.LocalCluster{
		Config: vttest.Config{
			...
			SchemaDir:     path.Join(os.Getenv("VTROOT"), "examples/demo1/schema"),
...
func exec(w http.ResponseWriter, req *http.Request) {
	...
	execQuery(conn, "corder_keyspace_idx0", "select * from corder_keyspace_idx", "customer", "-80", response)
	execQuery(conn, "corder_keyspace_idx1", "select * from corder_keyspace_idx", "customer", "80-", response)
	...
	execQuery(conn, "name_keyspace_idx", "select * from name_keyspace_idx", "product", "0", response)
	enc.Encode(response)
}
