...
func TestPiiFilter(t *testing.T) {
	...
	time.Sleep(1 * time.Second) //wait for copy to be completed

	require.Empty(t, validateCount(t, vtgateConn, "pii2:0", "pii_test", 1))
	...
	execVtgateQuery(t, vtgateConn, "pii:0", insert)
	time.Sleep(1 * time.Second) //wait for replication to find this row

	require.Empty(t, validateCount(t, vtgateConn, "pii2:0", "pii_test", 2))
	require.Empty(t, validateQuery(t, vtgateConn, "pii2:0",
