...
func TestPartialMoveTables(t *testing.T) {
	...
	expectedShardRoutingRules := `{"rules":[{"from_keyspace":"customer","to_keyspace":"customer2","shards":"80-"}]}`
	applyShardRoutingRules(t, expectedShardRoutingRules)

	validateCount(t, vtgateConn, "customer", "customer", 3)      //customer: all shards
	validateCount(t, vtgateConn, "customer2", "customer", 3)     //customer: all shards
	validateCount(t, vtgateConn, "customer2:80-", "customer", 2) // customer2: 80-

	validateQuery(t, vtgateConn, "customer:80-", "select name from customer where cid = 3", `[[VARBINARY("ringo")]]`)
	...
	validateQuery(t, vtgateConn, "customer", "select name from customer where cid = 3", `[[VARBINARY("ringo")]]`)
	// uses route to customer2:80-
	validateQuery(t, vtgateConn, "customer:80-", "select name from customer where cid = 3", `[[VARBINARY("Ringo Starr")]]`)
	...
	validateQuery(t, vtgateConn, "customer:80-", "select name from customer where cid = 3", `[[VARBINARY("Squaro Planett")]]`)
