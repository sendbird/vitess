...
type VtgateProcess struct {
	...
	Directory             string
	VerifyURL             string
	//Extra Arg to be set before starting the vtgate process
	ExtraArg []string
...
func (vtgate *VtgateProcess) Setup() (err error) {
	...
	vtgate.proc.Args = append(vtgate.proc.Args, vtgate.ExtraArg...)
...
func VtgateProcessInstance(Port int, GrpcPort int, MySQLServerPort int, Cell string, CellsToWatch string, Hostname string, TabletTypesToWait string, topoPort int, hostname string, ExtraArgs []string) *VtgateProcess {
