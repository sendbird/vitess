...
func TestLimitExecute(t *testing.T) {
	...
	result, err := l.Execute(ctx, nil, bindVars, false)
	...
	result, err = l.Execute(ctx, nil, map[string]*querypb.BindVariable{"l": sqltypes.Int64BindVariable(2)}, false)
	require.NoError(t, err)
	if !reflect.DeepEqual(result, wantResult) {
		t.Errorf("l.Execute:\n%v, want\n%v", result, wantResult)
	}
...
func TestLimitOffsetExecute(t *testing.T) {
	...
	result, err = l.Execute(ctx, nil, map[string]*querypb.BindVariable{"l": sqltypes.Int64BindVariable(1), "o": sqltypes.Int64BindVariable(1)}, false)
	require.NoError(t, err)
	if !reflect.DeepEqual(result, wantResult) {
		t.Errorf("l.Execute:\n got %v, want\n%v", result, wantResult)
	}
...
func TestLimitStreamExecute(t *testing.T) {
	...
	err := l.StreamExecute(ctx, nil, bindVars, false, func(qr *sqltypes.Result) error {
	...
	results = nil
	err = l.StreamExecute(ctx, nil, map[string]*querypb.BindVariable{"l": sqltypes.Int64BindVariable(2)}, false, func(qr *sqltypes.Result) error {
		results = append(results, qr)
		return nil
	})
	require.NoError(t, err)
...
func TestLimitInputFail(t *testing.T) {
	...
	want := "input fail"
	if _, err := l.Execute(ctx, nil, bindVars, false); err == nil || err.Error() != want {
		t.Errorf("l.Execute(): %v, want %s", err, want)
	}

	fp.rewind()
	err := l.StreamExecute(ctx, nil, bindVars, false, func(_ *sqltypes.Result) error { return nil })
...
func TestLimitInvalidCount(t *testing.T) {
	...
	_, err = l.Execute(ctx, nil, nil, false)
	if err == nil || err.Error() != want {
		t.Errorf("l.Execute: %v, want %s", err, want)
	}

	err = l.StreamExecute(ctx, nil, nil, false, func(_ *sqltypes.Result) error { return nil })
	if err == nil || err.Error() != want {
		t.Errorf("l.Execute: %v, want %s", err, want)
	}
}
