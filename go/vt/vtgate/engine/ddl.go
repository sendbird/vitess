...
func (ddl *DDL) TryExecute(vcursor VCursor, bindVars map[string]*query.BindVariable, wantfields bool, _ *RoutingParameters) (result *sqltypes.Result, err error) {
	if ddl.CreateTempTable {
...
func (ddl *DDL) TryStreamExecute(vcursor VCursor, bindVars map[string]*query.BindVariable, wantfields bool, callback func(*sqltypes.Result) error) error {
	results, err := ddl.TryExecute(vcursor, bindVars, wantfields, nil)
	if err != nil {
		return err
	}
