...
func (v *AlterVSchema) TryExecute(vcursor VCursor, bindVars map[string]*query.BindVariable, wantfields bool, _ *RoutingParameters) (*sqltypes.Result, error) {
...
func (v *AlterVSchema) TryStreamExecute(vcursor VCursor, bindVars map[string]*query.BindVariable, wantfields bool, callback func(*sqltypes.Result) error) error {
	res, err := v.TryExecute(vcursor, bindVars, wantfields, nil)
	if err != nil {
		return err
	}
	return callback(res)
