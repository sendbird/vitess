...
func (upd *Update) TryExecute(vcursor VCursor, bindVars map[string]*querypb.BindVariable, wantfields bool, _ *RoutingParameters) (*sqltypes.Result, error) {
...
func (upd *Update) TryStreamExecute(vcursor VCursor, bindVars map[string]*querypb.BindVariable, wantfields bool, callback func(*sqltypes.Result) error) error {
	res, err := upd.TryExecute(vcursor, bindVars, wantfields, nil)
	if err != nil {
		return err
	}
	return callback(res)
