...
func TestLookupUniqueMap(t *testing.T) {
	...
	got, err := lookupUnique.Map(ctx, vc, []sqltypes.Value{sqltypes.NewInt64(1), sqltypes.NewInt64(2)})
	...
	vc.numRows = 2
	_, err = lookupUnique.Map(ctx, vc, []sqltypes.Value{sqltypes.NewInt64(1), sqltypes.NewInt64(2)})
	...
	// Test query fail.
	vc.mustFail = true
	_, err = lookupUnique.Map(ctx, vc, []sqltypes.Value{sqltypes.NewInt64(1)})
...
func TestLookupUniqueVerify(t *testing.T) {
	...
	_, err := lookupUnique.Verify(ctx, vc, []sqltypes.Value{sqltypes.NewInt64(1)}, [][]byte{[]byte("test")})
	require.NoError(t, err)
...
func TestLookupUniqueVerifyWriteOnly(t *testing.T) {
	...
	got, err := lookupUnique.Verify(ctx, vc, []sqltypes.Value{sqltypes.NewInt64(1)}, [][]byte{[]byte("test")})
	require.NoError(t, err)
	want := []bool{true}
	if !reflect.DeepEqual(got, want) {
...
func TestLookupUniqueCreate(t *testing.T) {
	...
	err = lookupUnique.(Lookup).Create(ctx, vc, [][]sqltypes.Value{{sqltypes.NewInt64(1)}}, [][]byte{[]byte("test")}, false)
	require.NoError(t, err)
...
func TestLookupUniqueDelete(t *testing.T) {
	...
	err := lookupUnique.(Lookup).Delete(ctx, vc, [][]sqltypes.Value{{sqltypes.NewInt64(1)}}, []byte("test"))
	require.NoError(t, err)
	if got, want := len(vc.queries), 1; got != want {
		t.Errorf("vc.queries length: %v, want %v", got, want)
	}
...
func TestLookupUniqueUpdate(t *testing.T) {
	...
	err := lookupUnique.(Lookup).Update(ctx, vc, []sqltypes.Value{sqltypes.NewInt64(1)}, []byte("test"), []sqltypes.Value{sqltypes.NewInt64(2)})
	require.NoError(t, err)
