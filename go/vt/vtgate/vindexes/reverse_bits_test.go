...
func TestReverseBitsMap(t *testing.T) {
	got, err := reverseBits.Map(ctx, nil, []sqltypes.Value{
		sqltypes.NewInt64(1),
		sqltypes.NewInt64(2),
		sqltypes.NewInt64(3),
		sqltypes.NULL,
...
func TestReverseBitsVerify(t *testing.T) {
	...
	got, err := reverseBits.Verify(ctx, nil, ids, ksids)
	if err != nil {
		t.Fatal(err)
	}
	...
	// Failure test
	_, err = reverseBits.Verify(ctx, nil, []sqltypes.Value{sqltypes.NewVarBinary("aa")}, [][]byte{nil})
