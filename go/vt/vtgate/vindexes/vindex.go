...
type Lookup interface {
	...
	Create(ctx context.Context, vc VCursor, rowsColValues [][]sqltypes.Value, ksids [][]byte, ignoreMode bool) error

	Delete(ctx context.Context, vc VCursor, rowsColValues [][]sqltypes.Value, ksid []byte) error

	// Update replaces the mapping of old values with new values for a keyspace id.
	Update(ctx context.Context, vc VCursor, oldValues []sqltypes.Value, ksid []byte, newValues []sqltypes.Value) error
}
