...
func (cp *Pool) Open(appParams, dbaParams, appDebugParams dbconfigs.Connector) {
	...
	cp.connections = pools.NewResourcePool(f, cp.capacity, cp.capacity, cp.idleTimeout, cp.refreshTimeout, cp.prefillParallelism, cp.getLogWaitCallback(), refreshCheck, *mysqlctl.PoolDynamicHostnameResolution)
...
// IdleClosed returns the number of connections closed to idle timeout for the pool.
