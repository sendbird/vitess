...
func (wr *Wrangler) masterMigrateServedFrom(ctx context.Context, sourceShard, destinationShard *topo.ShardInfo, tables []string, ev *events.MigrateServedFrom, filteredReplicationWaitTime time.Duration, reverseReplication bool, isEmergency bool) error {
	...
	if len(destinationShard.SourceShards) != 0 {
		...
		wr.Logger().Infof("Reached point of no return")
	}
	...
	// Best effort clean up of vreplication.
	if uid != 0 {
		if _, err := wr.tmc.VReplicationExec(ctx, destinationMasterTabletInfo.Tablet, binlogplayer.DeleteVReplication(uid)); err != nil {
			wr.Logger().Errorf("Could not delete vreplication for uid %d, please do so manually: %v", uid, err)
		}
	}
	if reverseReplication {
